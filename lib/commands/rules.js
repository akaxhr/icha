...
import { editMessage } from "../settings/telegramUi.js";
...
export async function handleRulesPreviewCallback(callbackQuery) {
  ...
  const groupTitle =
    callbackQuery.message.reply_markup?.inline_keyboard
      ?.flat()
      ?.find((b) => b.callback_data === callbackQuery.data)
      ?.text || groupId;
...
${rules?.text || rules?.media_caption || "No regulation set."}`,
    rules?.buttons_json?.length
      ? { inline_keyboard: rules.buttons_json }
      : {
          inline_keyboard: [
            [{ text: "⬅️ Back", callback_data: "rulespreview:back" }]
          ]
        }
  );
